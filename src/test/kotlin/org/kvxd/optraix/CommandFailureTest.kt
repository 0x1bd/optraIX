...
class CommandFailureTest {
    ...
    private class BoomCommand : ServerCommand {
        ...
        override fun register(dispatcher: com.mojang.brigadier.CommandDispatcher<CommandSource>) {
            dispatcher.register(literal(name).runs { throw NoClassDefFoundError("org/kvxd/optraix/Missing") })
        }
    }
    ...
    fun failingCommandLogsAStackTraceAndTellsThePlayer() {
        val server = OptraIxServer(ServerConfig(port = 0, runDirectory = File("build/tmp/failure-test")))
