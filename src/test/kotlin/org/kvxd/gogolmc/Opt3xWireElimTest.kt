...
class Opt3xWireElimTest {

    private val fuzz = Opt3xFuzzTest()

    private val buildMethod = Opt3xFuzzTest::class.java
        .getDeclaredMethod("build", Long::class.java, Int::class.java, Int::class.java)
        .apply { isAccessible = true }

    private val settleMethod = Opt3xFuzzTest::class.java
    ...
    private fun check(seed: Long, rows: Int, length: Int, ticks: Int): Pair<Int, Int> {
        ...
        val exact = Opt3xCompiler.compile(candidateWorld, eliminateWire = false)
        val circuit = Opt3xCompiler.compile(candidateWorld, eliminateWire = true)
        circuit.settle()
    ...
    fun busCircuitCollapsesToGates() {
        val circuit = org.kvxd.gogolmc.bench.BenchCircuit.busses(6, 8)
        val compiled = Opt3xCompiler.compile(circuit.world, eliminateWire = true)
        assertTrue(
