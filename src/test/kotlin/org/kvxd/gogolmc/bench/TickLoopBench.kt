...
object TickLoopBench {
    ...
    fun main(args: Array<String>) {
        ...
        val compiled = org.kvxd.gogolmc.redstone.opt3x.Opt3xCompiler.compile(circuit.world)
        compiled.settle()
        repeat(200) { compiled.tick() }
        println("idle circuit: ${compiled.count} nodes, ${compiled.pendingTicks} pending")
        measure("opt3x idle tick") { compiled.tick() }
        measure("opt3x idle tick + housekeeping") {
            compiled.tick()
            if (it and 255 == 0) {
                while (true) {
                    tasks.poll() ?: break
