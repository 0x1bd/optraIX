...
class PersistenceTest {
    ...
    private fun tempFile(): File {
        val directory = Files.createTempDirectory("gogolmc").toFile()
    ...
    private fun emptyWorld() = GameWorld(org.kvxd.gogolmc.world.WorldGenerator(Blocks.airState, 0))

    private fun tick(world: GameWorld, times: Int) {
        repeat(times) {
            world.tickScheduled { pos -> org.kvxd.gogolmc.redstone.mchprs.MchprsRedstone.tick(world, pos) }
        }
    }
    ...
    fun pendingTorchTickSurvivesReload() {
        ...
        val interaction = org.kvxd.gogolmc.interaction.Interaction(
            org.kvxd.gogolmc.redstone.mchprs.MchprsRedstone
        )
        ...
        interaction.placeInWorld(
            BlockStates.leverState(
                org.kvxd.gogolmc.block.property.LeverFace.Wall, BlockDirection.East, false
            ),
            world, BlockPos(2, 0, 0), null,
        )
        org.kvxd.gogolmc.redstone.mchprs.MchprsRedstone.onUse(world, BlockPos(2, 0, 0))
    ...
    fun pendingRepeaterDelaySurvivesReload() {
        ...
        interaction.placeInWorld(
            BlockStates.leverState(
                org.kvxd.gogolmc.block.property.LeverFace.Floor, BlockDirection.North, false
            ),
            world, BlockPos(0, 1, 0), null,
        )
        org.kvxd.gogolmc.redstone.mchprs.MchprsRedstone.onUse(world, BlockPos(0, 1, 0))
    ...
    fun loadingMissingFileIsNoop() {
        val world = GameWorld()
        assertEquals(0, WorldStorage.load(world, File("/nonexistent/gogolmc.world")))
    }
    ...
    fun editsAfterLoadStillWork() {
        ...
        val wire = BlockStates.wireState(
            org.kvxd.gogolmc.block.property.WireSide.Side,
            org.kvxd.gogolmc.block.property.WireSide.Side,
            org.kvxd.gogolmc.block.property.WireSide.Side,
            org.kvxd.gogolmc.block.property.WireSide.Side,
            9,
